...
//                    node registry (so any node registered for Galdr — including
//                    Faust and future host nodes — is loadable for free)
//   to_dot(bp)    -> Graphviz DOT for visual inspection (export only)
//
// All of this is CONTROL-THREAD code: it allocates, and must never run on the
// audio thread.
//
...
inline json::value to_json_value(const graph_blueprint& bp, const std::string& name = "") {
...
inline std::string to_json(const graph_blueprint& bp, const std::string& name = "", int indent = 2) {
...
inline bool from_json(const std::string& text,
    ...
    for (const json::value& jn : jnodes->arr) {
        ...
        galdr::node_args args;
        if (const json::value* ja = jn.find("args"); ja && ja->is_object()) {
            for (const auto& kv : ja->obj) {
                const json::value& v = kv.second;
                galdr::arg_value av;
                if (v.is_number())      av = galdr::arg_value(v.number);
                else if (v.is_string()) av = galdr::arg_value::make_string(v.str);
                else if (v.k == json::value::kind::boolean)
                                        av = galdr::arg_value::make_bool(v.boolean);
    ...
    auto load_terminals = [&](const char* key, std::vector<terminal>& dst) -> bool {
        ...
        for (const json::value& t : jt->arr) {
            ...
            dst.push_back({t.text("name"), size_t(nd)});
        }
        return true;
    };
...
inline std::string trim_zeros(double v) {
    char buf[32];
...
inline std::string to_dot(const graph_blueprint& bp, const std::string& name = "xrune") {
    std::string o;
